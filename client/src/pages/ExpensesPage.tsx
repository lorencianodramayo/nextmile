...
export default function ExpensesPage() {
  ...
  const categoryOptions = useMemo(() => {
    const allCats = [...new Set([...expenseCategories, ...DEFAULT_CATEGORIES])].sort();
  ...
  useKeyboardShortcuts({ onNewExpense: openAdd });
  ...
  const handleSave = async () => {
    if (!selectedTruck) { setShowTruckWarning(true); return; }
    if (!form.date) { toast.error('Date is required.', { duration: 6000 }); return; }
    if (!form.category.trim()) { toast.error('Category is required.', { duration: 6000 }); return; }
    if (!form.amount) { toast.error('Amount is required.', { duration: 6000 }); return; }
    setLoading(true);
    try {
    ...
    } catch (err: unknown) { toast.error(err instanceof Error ? err.message : 'Failed to save expense', { duration: 7000 }); }
                  ...
                  <tr><td colSpan={5}>
                    <EmptyState
                      ...
                      action={selectedTruck ? (
                        <button onClick={openAdd} className="px-4 py-2.5 rounded-[14px] bg-gradient-to-br from-blue-600 to-blue-700 text-white text-sm font-semibold shadow-[0_10px_20px_rgba(37,99,235,0.18)] hover:from-blue-700 hover:to-blue-800 transition-all flex items-center gap-1.5">
          ...
          <div className="flex flex-col gap-3 md:hidden p-3 border-t border-slate-200/60 dark:border-slate-700/60">
            {filteredRows.length === 0 ? (
              <EmptyState
                ...
                action={selectedTruck ? (
                  <button onClick={openAdd} className="px-4 py-2.5 rounded-[14px] bg-gradient-to-br from-blue-600 to-blue-700 text-white text-sm font-semibold flex items-center gap-1.5">
                    <Plus size={16} /> Add Expense
                  </button>
                ) : undefined}
              />
