...
export default function TripsPage() {
  ...
  const [driverMonth, setDriverMonth] = useState('ALL');
  const [driverStatus, setDriverStatus] = useState<'ALL' | 'UNPAID' | 'PAID'>('ALL');
  ...
  const filteredRows = useMemo(() => {
    let rows = tripRows;
    // Driver month filter
    if (!admin && driverMonth !== 'ALL') {
      rows = rows.filter((r) => {
        const d = new Date(r.dateIso);
        return String(d.getMonth() + 1) === driverMonth;
      });
    }
    ...
    return rows;
  }, [tripRows, searchQuery, sortField, sortDirection, admin, driverMonth, driverStatus]);
  ...
  return (
    <div>
      ...
      <FilterBar
        ...
        showRange={admin}
        showMonth={!admin}
        monthValue={!admin ? driverMonth : undefined}
        onMonthChange={!admin ? setDriverMonth : undefined}
        actions={
