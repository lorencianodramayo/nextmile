...
export default function PaymentsPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      formData.append('date', `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`);
    ...
    <div>
      <div className="glass-card rounded-[28px] border border-slate-200/80 dark:border-slate-700/90 shadow-lg p-5 mb-3.5">
        <div className="flex flex-col md:flex-row justify-between md:items-end gap-3">
          <div>
            <h1 className="text-[1.45rem] font-bold tracking-tight">Payments</h1>
            <p className="text-sm text-slate-500 mt-1">Upload GCash screenshots for Cash Advance and Salary payments.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-3">
        ...
        <div className="glass-card rounded-[22px] border border-slate-200/90 dark:border-slate-700/90 shadow-sm p-5">
          <h2 className="text-base font-bold tracking-tight mb-4">Upload Screenshot</h2>
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              <Select
                ...
                onChange={(opt) => { if (opt) setCategory(opt.value); }}
            ...
            <div>
              <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1.5 block">Note (optional)</label>
              <input
                type="text"
                value={note}
            ...
            <div>
              <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1.5 block">Screenshot</label>
              <label className="flex flex-col items-center justify-center w-full min-h-[120px] rounded-[14px] border-2 border-dashed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 cursor-pointer hover:border-blue-400 dark:hover:border-blue-500 transition-colors">
                ...
                {file ? (
                  <div className="text-center p-4">
                    ...
                    <div className="text-xs font-semibold text-slate-700 dark:text-slate-300 truncate max-w-[200px]">{file.name}</div>
                    <div className="text-[0.65rem] text-slate-400 mt-1">{(file.size / 1024).toFixed(1)} KB</div>
                  </div>
                ) : (
                  <div className="text-center p-4">
                    <Upload size={24} className="mx-auto mb-2 text-slate-400" />
                    <div className="text-xs text-slate-500">Click to upload screenshot</div>
                  </div>
                )}
              </label>
            </div>
            ...
            {file && (
              <div className="text-xs text-slate-500 bg-slate-50 dark:bg-slate-800 rounded-xl p-3">
                <span className="font-semibold">Filename: </span>{filenamePreview}
              </div>
            )}
        ...
        {/* Gallery */}
        <div className="glass-card rounded-[22px] border border-slate-200/90 dark:border-slate-700/90 shadow-sm p-5">
          <h2 className="text-base font-bold tracking-tight mb-1">Uploaded Screenshots</h2>
          <p className="text-sm text-slate-500 mb-4">{selectedTruckName || 'All trucks'}</p>
          ...
          {payments.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {payments.map((p) => (
                <div key={p._id} className="glass-card rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                  <div
                    className="h-40 bg-slate-100 dark:bg-slate-800 cursor-pointer flex items-center justify-center"
                    onClick={() => window.open(`/api/payments/${p._id}/file`, '_blank')}
                  >
                    <img
                  ...
                  <div className="p-3">
                    <div className="font-semibold text-xs truncate">{p.filename}</div>
                    <div className="text-[0.65rem] text-slate-500 mt-0.5">{p.dateText}</div>
                    <div className="flex items-center justify-between mt-2">
                      <span className="inline-block px-2 py-0.5 rounded-full text-[0.65rem] font-bold bg-blue-500/10 text-blue-600 dark:text-blue-400">{p.category}</span>
                      <button
                    ...
                    {p.note && <div className="text-[0.65rem] text-slate-400 mt-1 truncate">{p.note}</div>}
                  </div>
                </div>
              ))}
            </div>
