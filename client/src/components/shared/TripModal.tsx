  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        crewSalary: Number(form.crewSalary) || 0,
        cashAdvance: Number(form.cashAdvance) || 0,
        reimbursements: Number(form.reimbursements) || 0,
        note: form.note,
      };
    ...
    >
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="number" value={form.cashAdvance} onChange={(e) => setForm({ ...form, cashAdvance: e.target.value })} placeholder="0" className={inputClass} />
        </div>
        <div className="col-span-2">
          <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1.5 block">Reimbursements (₱)</label>
          <input type="number" value={form.reimbursements} onChange={(e) => setForm({ ...form, reimbursements: e.target.value })} placeholder="0" className={inputClass} />
        </div>
        <div className="col-span-2">
          <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1.5 block">Note</label>
          <textarea value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} className={inputClass + ' min-h-[80px] py-2.5 resize-none'} rows={3} placeholder="Add notes..." />
        </div>
      </div>
    </Modal>
  );
